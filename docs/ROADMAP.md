...
3. **Deferred cloud-save:** `Lineup` (persistent, minor) + `MatchRecord` (slate-transient) — optional; low value.
...
- **Tooling**: XcodeGen project, `tools/player_manifest.csv` + `tools/build_catalog.py` (catalog authoring), `tools/fetch_flags.sh`, dev launch args, 64 passing tests.
